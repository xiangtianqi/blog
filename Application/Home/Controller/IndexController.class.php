...
class IndexController extends BaseController
{
    public function index()
    {
        ...
        $open['open'] = 1;
        $count = $article->where($open)->count();
    ...
    public function article()
    {
        ...
        if ($a) {
            $article->where("id = {$id}")->setInc('view', 1); //访问量+1
    ...
    public function part(){
        ...
        $article = D('article');
        $cate_gory=D('category');
        $open['open'] = 1;
        $id = I('id');
        //此写法只适合栏目层级为2级
        $ids=$cate_gory->where("tid={$id}")->field('id')->select();
        if(!empty($ids)){
            $idmun="";
